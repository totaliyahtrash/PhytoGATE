"""
PhytoGATE Benchmark Runner: PlantDoc In-the-Wild Field Benchmark
"""
...
def load_plantdoc_dataset(dataset_dir="./plantdoc_leaf_dataset"):
    print("[+] Loading PlantDoc Field Dataset...")
    images, labels = [], []
    zip_url = "https://github.com/pratikkayal/PlantDoc-Dataset/archive/refs/heads/master.zip"
    zip_target_path = "./pd_temp.zip"
    ...
    for class_idx, class_name in enumerate(CLASS_NAMES):
        class_files = []
        name_clean = class_name.lower().replace(' ', '').replace('_', '')
...
def main():
    print("==================================================================")
    print("     PhytoGATE BENCHMARK SUITE: PlantDoc In-The-Wild Field        ")
    print("==================================================================")
    images_rgb, labels = load_plantdoc_dataset()
    if len(images_rgb) == 0:
    ...
    for seed in SEEDS:
        ...
        print(f"[OK] Seed {seed} PlantDoc Test Accuracy: {acc:.2f}% (F1: {f1:.4f})")

    arr = np.array(results)
    print("\n==================================================================")
    print(f"PhytoGATE PlantDoc Field Mean Accuracy: {np.mean(arr[:,0]):.2f}% ± {np.std(arr[:,0]):.2f}% (Peak: {np.max(arr[:,0]):.2f}%)")
    print("==================================================================")
